...
public class App implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent event, Context context) {
        ...
        return generateResponse(data, event.getQueryStringParameters().get(QUERY_PARAM_NAME));
    }

    protected static String readData(APIGatewayProxyRequestEvent event) {
        Map<String,String> queryParams = event.getQueryStringParameters();
        SelectObjectContentRequest request = generateJSONRequest(queryParams);
    ...
    private static SelectObjectContentRequest generateJSONRequest(Map<String,String> queryParams) {
        SelectObjectContentRequest request = new SelectObjectContentRequest();
        request.setBucketName(S3_BUCKET);
        request.setKey(S3_FILE);
        request.setExpression("select s.review_headline, s.review_body from s3object[*] s where s.product_id = '" + queryParams.get(QUERY_PARAM_NAME) + "'");
